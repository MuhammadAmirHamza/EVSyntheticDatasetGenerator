...
class exponential_process:
            ...
            for day_i in np.unique(day):
                ...
                for t_i in np.unique(x):
                    try:
                        ...
                        continue
                if len(model_d_temp) == 0:
                    continue
                model_d_temp = np.vstack(model_d_temp)
                mask = (model_d_temp[:, 0] == combine[0])
                if mask.any():
                    scale_val = model_d_temp[mask].flatten()[1]
        ...
        for f2 in np.unique(x):
            ...
            if self.combined_slots and f2 == self.combine[0]:
                for var in self.combine:
                    ...
                    exp_scale = pd.concat([exp_scale, pd.DataFrame([scale])], ignore_index=True)
    ...
    def predict_day(self, X_test, Start_time, slot,
        ...
        t_now = float(np.asarray(Start_time).flatten()[0])
        ts = []
        ...
        while t_now <= 24.00:
            ...
            scale_val = float(np.asarray(scale).flatten()[0])
            if self._scale_min > scale_val:
                self._scale_min = scale_val

            t_diff = float(np.random.exponential(scale_val, size=1)[0])
            # update t_now
            t_now = t_now + t_diff
