# =============================================================================
# smc_core.jl — SHARED SMC framework body (single source of truth)
# -----------------------------------------------------------------------------
# This is SMC_framework_norway.jl's body with ONLY the 7 per-dataset values
# externalized. Do NOT run directly — it is `include`d by run_smc.jl, which must
# define these globals BEFORE the include:
#     DATA_PATH, TOP_N_USERS, Q_UPPER, UPPER_INCLUSIVE, NUM_BINS, A_KMAX, ARTIFACTS_DIR
# Everything else (N_MIN=30, G_KMAX=6, L2_MC=2000, Random.seed!(24), all logic)
# is byte-identical to the per-dataset scripts, so results are reproduced exactly.
# =============================================================================
...
const L2_MC          = 2000         # Monte-Carlo replicates for the L2 transition test
const G_KMAX         = 6            # max truncated-GMM components for the idle-gap (G) fit
...
counts = UPPER_INCLUSIVE ? counts[(counts.n_u .>= N_MIN) .& (counts.n_u .<= upper), :] : counts[(counts.n_u .>= N_MIN) .& (counts.n_u .< upper), :]
sort!(counts, :n_u, rev=true)
if nrow(counts) == 0
    error("No users satisfy N_MIN=$N_MIN and < $(floor(Int, upper)) sessions/user; " *
          "data too sparse — lower N_MIN/TOP_N_USERS or check the input.")
end
n_sel = min(TOP_N_USERS, nrow(counts))
top_uids = first(counts.user_id, n_sel)
...
@printf("  Session count range   : %d – %d (median %d)\n",
         minimum(counts.n_u[1:n_sel]),
         maximum(counts.n_u[1:n_sel]),
         floor(median(counts.n_u[1:n_sel])))
...
# idle-gap cap (hours). Injected by run_smc.jl via G_EFF_MAX_HOURS_RUN;
# defaults to 10 days when run standalone (preserves original results).
const G_EFF_MAX_HOURS = (@isdefined(G_EFF_MAX_HOURS_RUN) ?
                         Float64(G_EFF_MAX_HOURS_RUN) : 10 * 24.0)
...
# [vine] safe scalar CDF / quantile: return NaN when the fitted dist does not
# support them (e.g. the wrapped multivariate truncated-GMM MixtureModel), so
# callers can fall back to an independent marginal draw instead of crashing.
_cdf1d(dist, x::Real) = try
    v = cdf(dist, x); (v isa Real && isfinite(v)) ? Float64(v) : NaN
catch; NaN end
_quantile1d(dist, u::Real) = try
    v = quantile(dist, clamp(u, 1e-6, 1 - 1e-6)); (v isa Real && isfinite(v)) ? Float64(v) : NaN
catch; NaN end
...
function fit_best(x::AbstractVector{<:Real};
    ...
    # [SPARC_vine] extended family set: + Gamma, InverseGaussian (safe, BIC-guarded).
    for (Fam, k) in [(LogNormal, 2), (Exponential, 1), (Weibull, 2), (Gamma, 2), (InverseGaussian, 2)]
        try
            f   = fit_mle(Fam, xx)
            ll  = sum(logpdf.(f, xx))
            bic = k * log(n) - 2 * ll
...
for sub in groupby(pairs, :day_type)
    ...
    global_fits[dt] = (
        D = fit_best(sub.D),
        G = fit_best(sub.G_eff; gmm_kmax=G_KMAX),
        E = fit_best(sub.E),
        A = fit_best(sub.A; gmm_lower=0.0, gmm_upper=24.0, gmm_kmax=A_KMAX),
    )
...
mutable struct UserDayProfile
    ...
    rho      :: Vector{Float64}           # D-G copula ρ (filled in Stage 3)
    rho_de   :: Vector{Float64}           # [vine] D-E copula ρ (energy-duration coupling)
...
for (uid, dt) in unique(zip(pairs.user_id, pairs.day_type))
    ...
    for b in 1:NUM_BINS
        ...
        g1 = nrow(bin_rows) >= N_BIN_NOMINAL ? fit_best(bin_rows.G_eff; gmm_kmax=G_KMAX) : nothing
        e1 = nrow(bin_rows) >= N_BIN_NOMINAL ? fit_best(bin_rows.E) : nothing
        a1 = nrow(bin_rows) >= N_BIN_NOMINAL ? fit_best(bin_rows.A; gmm_lower=0.0, gmm_upper=24.0, gmm_kmax=A_KMAX) : nothing
    ...
    profiles[(uid, dt)] = UserDayProfile(
        ...
        fill(NaN, NUM_BINS),     # [vine] rho_de: D-E copula, filled in Stage 3
...
for ((uid, dt), prof) in profiles
    ...
    for b in 1:NUM_BINS
        ...
        # --- [vine] Unconditional D-E dependence: Gaussian-copula ρ from Spearman rank
        #     correlation (empirical -> no CDF needed, so it is defined for GMM-fit bins too).
        prof.rho_de[b] = NaN
        try
            if length(d_emp) > 3
                e_emp = Float64.(bin_rows.E)
                rs = corspearman(d_emp, e_emp)
                if !isnan(rs)
                    prof.rho_de[b] = clamp(2 * sin(pi * rs / 6), -0.99, 0.99)
                end
            end
        catch
            prof.rho_de[b] = NaN
...
for uid in unique(raw.user_id)
    ...
    for dt in ["Weekday", "Weekend"]
        ...
        b     = rand(Distributions.Categorical(prof.pi0))
        t_eff = 0.0
        ...
        while true
            ...
            # --- [vine] Sample (D, G, E): D-anchored Gaussian copula; E coupled to D when
            #     the marginals support CDF/quantile, else graceful fallback to a marginal draw ---
            if d_dist === nothing || g_dist === nothing
                d_n, g_n = 1.0, 1.0
                e_n = e_dist === nothing ? 0.01 : max(sample_1d(e_dist), 0.01)
            else
                d_n = max(sample_1d(d_dist), 0.01)
                ud  = _cdf1d(d_dist, d_n)
                z_d = isnan(ud) ? NaN : quantile(Normal(), clamp(ud, 1e-6, 1 - 1e-6))
                anchor = !isnan(z_d)                 # copula usable only if D's CDF is defined
                # G: couple to D only on failing bins (unchanged), and only if usable
                if !anchor || prof.bin_pass[b] || isnan(prof.rho[b])
                    g_n = max(sample_1d(g_dist), 0.0)
                else
                    ρ = prof.rho[b]
                    z_g = ρ * z_d + sqrt(1 - ρ^2) * randn()
                    gq  = _quantile1d(g_dist, cdf(Normal(), z_g))
                    g_n = isnan(gq) ? max(sample_1d(g_dist), 0.0) : max(gq, 0.0)
                end
                # E: couple to D unconditionally when usable (fixes the D-E dependence)
                if e_dist === nothing
                    e_n = 0.01
                elseif !anchor || isnan(prof.rho_de[b])
                    e_n = max(sample_1d(e_dist), 0.01)
                else
                    ρe  = prof.rho_de[b]
                    z_e = ρe * z_d + sqrt(1 - ρe^2) * randn()
                    eq  = _quantile1d(e_dist, cdf(Normal(), z_e))
                    e_n = isnan(eq) ? max(sample_1d(e_dist), 0.01) : max(eq, 0.01)
                end
            end
            ...
            # --- Sample arrival hour (energy already drawn jointly with D above) ---
            ...
            # [vine] e_n drawn jointly with D above (D-E coupling); no marginal redraw
            ...
            # --- Next bin ---
            b     = rand(Distributions.Categorical(prof.P[b, :]))
            t_eff = t_eff_next
        end
    end
end
...
#            artifacts/validation_l4.csv    (population KL, multi-variable)
#            artifacts/kl_divergence.txt    (detailed L4 breakdown)
#
# Four-level hierarchical validation:
#   L1 — Marginal KS on {A, D, G_eff, E} per profile
#   L2 — Transition matrix χ² per profile
#   L3 — sojourn time distribution using KS test
#   L4 — Population-level KL divergence on {A, D, G_eff, E}
# =============================================================================
...
for sub_real in groupby(pairs, [:user_id, :day_type])
    ...
    dof  = max(dof - rows_used, 1)   # retained for reference only

    # --- Monte-Carlo p-value (calibrated for concentrated / low-count matrices) ---
    # Under H0 the simulated chain transitions follow the fitted real row
    # probabilities p_real. Draw L2_MC null chains with the same per-row visit
    # counts, recompute χ², and take the empirical exceedance probability.
    # Robust at any sample size — no large-count or dof asymptotics.
    χ2_obs = χ2
    n_ge   = 0
    for _ in 1:L2_MC
        χ2_null = 0.0
        for b in 1:NUM_BINS
            rt = sum(N_real[b, :])
            rt <= ALPHA_LAP * NUM_BINS && continue
            st_int = round(Int, sum(N_sim[b, :]) - ALPHA_LAP * NUM_BINS)
            st_int <= 0 && continue
            p_real   = N_real[b, :] ./ rt
            expected = p_real .* (st_int + ALPHA_LAP * NUM_BINS)
            draws    = rand(Multinomial(st_int, p_real)) .+ ALPHA_LAP
            for b2 in 1:NUM_BINS
                expected[b2] < 0.5 && continue
                χ2_null += (draws[b2] - expected[b2])^2 / expected[b2]
            end
        end
        χ2_null >= χ2_obs && (n_ge += 1)
    end
    pval = (1 + n_ge) / (1 + L2_MC)
...
# =============================================================================
# =============================================================================
# L3 — Sojourn time distribution KS test (per user, per day-type)
# =============================================================================
# Two-sample KS test on τ_eff = D + G_eff between real consecutive pairs
# and simulated inter-arrival times. Directly tests the fitted sojourn model
# — the core of the SMC — with no binning and no horizon dependence.
# =============================================================================

println("  Running L3 (sojourn time KS, per user) ...")
...
for sub_real in groupby(pairs, [:user_id, :day_type])
    ...
    real_tau = Float64.(sub_real.tau_eff)
    sim_tau  = Float64.(sub_sim.tau_eff_h)

    length(real_tau) < N_OBS_VAL && continue
    length(sim_tau)  < N_OBS_VAL && continue

    ks   = ApproximateTwoSampleKSTest(real_tau, sim_tau)
    pval = pvalue(ks)

    push!(l3_rows, (
        user_id  = uid,
        day_type = dt,
        n_real   = length(real_tau),
        n_sim    = length(sim_tau),
        ks_stat  = round(ks.δ,  digits = 4),
        p_value  = round(pval,  digits = 4),
        passed   = pval > KS_ALPHA,
    ))
...
l3 = DataFrame(l3_rows)
CSV.write(joinpath(ARTIFACTS_DIR, "validation_l3.csv"), l3)

# =============================================================================
# L4 — Population-level KL divergence (multi-variable)
# =============================================================================
println("  Running L4 (population KL, multi-variable) ...")

# Helper: compute KL divergence from two vectors using fixed-bin histograms
function population_kl(real_vals::Vector{Float64}, sim_vals::Vector{Float64};
                       n_bins::Int = 50, eps::Float64 = KL_EPS)
    lo = min(minimum(real_vals), minimum(sim_vals))
    hi = max(maximum(real_vals), maximum(sim_vals))
    hi == lo && (hi = lo + 1.0)
    edges = range(lo, hi; length = n_bins + 1)
    real_hist = Float64[count(x -> edges[k] <= x < edges[k+1], real_vals) for k in 1:n_bins]
    sim_hist  = Float64[count(x -> edges[k] <= x < edges[k+1], sim_vals)  for k in 1:n_bins]
    # Put anything exactly equal to hi into the last bin
    real_hist[end] += count(==(hi), real_vals)
    sim_hist[end]  += count(==(hi), sim_vals)
    # Laplace smoothing
    p = (real_hist .+ 1.0) ./ (sum(real_hist) .+ n_bins)
    q = (sim_hist  .+ 1.0) ./ (sum(sim_hist)  .+ n_bins)
    return sum(p .* log.((p .+ eps) ./ (q .+ eps)))
end

# Special case for arrival hour: 24 clock-hour bins (as before)
function arrival_hour_kl(real_hours_raw, sim_hours_raw; eps = KL_EPS)
    real_h = [floor(Int, h) % 24 for h in real_hours_raw]
    sim_h  = [floor(Int, h) % 24 for h in sim_hours_raw]
    real_hist = Float64[count(==(h), real_h) for h in 0:23]
    sim_hist  = Float64[count(==(h), sim_h)  for h in 0:23]
    p = real_hist ./ sum(real_hist)
    q = sim_hist  ./ sum(sim_hist)
    return sum(p .* log.((p .+ eps) ./ (q .+ eps))), p, q
end

# Compute L4 for each variable
kl_arrival, real_dist_A, sim_dist_A = arrival_hour_kl(raw.arrival_hour, sim_df.arrival_hour)
kl_duration  = population_kl(Float64.(pairs.D),     Float64.(sim_df.duration_h))
kl_gap       = population_kl(Float64.(pairs.G_eff),  Float64.(sim_df.gap_h))
kl_energy    = population_kl(Float64.(pairs.E),      Float64.(sim_df.energy_kwh))

l4_results = DataFrame(
    variable = ["Arrival hour", "Plug-in duration", "Idle gap", "Energy"],
    short    = ["A", "D", "G", "E"],
    KL_nats  = round.([kl_arrival, kl_duration, kl_gap, kl_energy]; digits = 6),
)

CSV.write(joinpath(ARTIFACTS_DIR, "validation_l4.csv"), l4_results)

# Also write the detailed arrival-hour breakdown (backward compatible)
open(joinpath(ARTIFACTS_DIR, "kl_divergence.txt"), "w") do io
    @printf(io, "L4 — Population-level KL divergence (nats)\n")
    @printf(io, "==========================================\n\n")
    for row in eachrow(l4_results)
        @printf(io, "  %-18s : %.6f nats", row.variable, row.KL_nats)
        if row.KL_nats < 0.05
            @printf(io, "  (excellent)\n")
        elseif row.KL_nats < 0.15
            @printf(io, "  (acceptable)\n")
        else
            @printf(io, "  (attention needed)\n")
        end
    end
    @printf(io, "\nArrival hour breakdown:\n")
    @printf(io, "Hour  Real     Sim\n")
    for h in 0:23
        @printf(io, "  %02d   %.4f   %.4f\n", h, real_dist_A[h+1], sim_dist_A[h+1])
    end
end

# Keep kl_div for backward compatibility in summary (use arrival hour)
kl_div = kl_arrival
...
for v in ["A", "D", "G", "E"]
    ...
    np  = sum(sub.passed); nt = nrow(sub)
    @printf("  L1 %-2s  : %3d / %3d pass (%5.1f%%)\n", v, np, nt, 100 * np / max(nt, 1))
end
@printf("  L2     : %3d / %3d pass (%5.1f%%)\n", sum(l2.passed), nrow(l2), 100 * sum(l2.passed) / max(nrow(l2), 1))
@printf("  L3     : %3d / %3d pass (%5.1f%%)\n", sum(l3.passed), nrow(l3), 100 * sum(l3.passed) / max(nrow(l3), 1))
println("  L4 (population-level KL divergence):")
for row in eachrow(l4_results)
    rating = row.KL_nats < 0.05 ? "(excellent)" : row.KL_nats < 0.15 ? "(acceptable)" : "(attention needed)"
    @printf("    %-18s : %.4f nats  %s\n", row.variable, row.KL_nats, rating)
end

println("\n  Saved → artifacts/validation_l1.csv")
println("  Saved → artifacts/validation_l2.csv")
println("  Saved → artifacts/validation_l3.csv  (sojourn time KS)")
println("  Saved → artifacts/validation_l4.csv  (population KL, multi-variable)")
println("  Saved → artifacts/kl_divergence.txt")

println("\nStage 5 complete.")
println("Check: L1 D and G pass rates — low rates suggest sojourn fits are off.")
println("Check: L3 failures — τ_eff mismatch means gap/duration fits need attention.")
println("Check: validation_l4.csv for population-level KL per variable.")
