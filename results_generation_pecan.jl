...
PLOTS_DIR     = joinpath(@__DIR__, "plots", "pecanstreet")
mkpath(PLOTS_DIR)
...
data_full = CSV.read(joinpath(@__DIR__, "dataset", "pecanstreet", "pecanstreet_sessions.csv"), DataFrame)
...
NUM_BINS_T = 24   # 1-hour bins, matches NUM_BINS = 24 in SMC_framework_pecan
...
bin_labels = [@sprintf("%02d:00", (b-1)*(24 ÷ NUM_BINS_T)) for b in 1:NUM_BINS_T]
...
NUM_BINS    = 24   # 1-hour bins, matches SMC_framework_pecan
...
bin_labels = [@sprintf("%02d:00", (b-1)*(24 ÷ NUM_BINS)) for b in 1:NUM_BINS]
...
# Read KL divergence (arrival-hour KL, used in the summary annotation)
l4_kl   = CSV.read(joinpath(ARTIFACTS_DIR, "validation_l4.csv"), DataFrame)
kl_val  = l4_kl[l4_kl.short .== "A", :KL_nats][1]
