...
const DATA_PATH   = joinpath(@__DIR__, "dataset", "pecanstreet", "pecanstreet_sessions.csv")
const TOP_N_USERS = 10           # 10 EV homes after excluding PS_661; n_sel caps at available
const N_MIN       = 30           # minimum sessions to keep a user
const Q_UPPER     = 1.0          # tiny cohort: no upper percentile cap (keep every home)
...
const A_KMAX         = 6            # max truncated-GMM components for the arrival-hour (A) fit

ARTIFACTS_DIR = joinpath(@__DIR__, "artifacts", "pecanstreet")
...
counts = counts[(counts.n_u .>= N_MIN) .& (counts.n_u .<= upper), :]   # <= : keep the top home (tiny cohort)
...
NUM_BINS    = 24          # 1-hour bins (finer arrival resolution; was 12 = 2-hour)
