...
def generate_sample(AM,MMc,MMe,horizon_start,horizon_end):
    ...
    start = 0.05
    fit_year = int(AM._fit_year)
    for i in range(0, number_of_wd):
        if config['verbose'] > 1: print( ' ------------------- Generating data for {} ----------------'.format(str(dates[i].date())))
        # Remap the calendar year onto the training year so factor lookups succeed
        gen_year = int(dates[i].year)
        factor_str = fac[i].replace(str(gen_year), str(fit_year)) if gen_year != fit_year else fac[i]
        X_test = pd.DataFrame([factor_str], columns=['Factor1'])
        ...
        # carry next day's start, clamped so we never skip a whole day
        start = float(np.asarray(t_next).flatten()[0]) - 24
        if start > 24:
            start = 0.05  # reset if rollover pushed past a whole day
        if arrivals.size == 0:
            continue  # no arrivals generated for this day; skip writing rows
        ...
        predicted_days = pd.DataFrame(ts)
