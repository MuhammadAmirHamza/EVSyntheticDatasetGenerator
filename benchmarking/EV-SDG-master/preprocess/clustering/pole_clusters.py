...
def pole_clustering(ses_clust_file_path,ses_clust_file_name):
    ...
    if os.path.exists(os.path.join(save_location,save_name)):
        ...
        if Cluster_by == 'Day_type':
            data_cluster = pv.drop('Charge_point', axis=1)
        if Cluster_by == 'Time_slot':
            data_cluster = pv1.drop('Charge_point', axis=1)
        if Cluster_by == 'Session_clusters':
            data_cluster = pv2.drop('Charge_point', axis=1)
        ...
        if Cluster_by == 'All':
            data_cluster = pole_properties.drop('Charge_point', axis=1)
