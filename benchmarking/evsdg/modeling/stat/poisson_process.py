...
class poisson_process:
    ...
    def avg_scale_pred(self,year,poles):
        poles_val = float(np.asarray(self.poles_fun([poles])).flatten()[0])
        return self.scale_multiplier_predictor.predict(np.array([[year, poles_val]]))
        ...
        for f2 in np.unique(data_gamma['arrivalslot']):
            ...
            if self.combined_slots and f2 == self.combine[0]:
                for var in self.combine:
                    pos_l = pd.concat([pos_l, pd.DataFrame([lam])], ignore_index=True)
                    exp_loc = pd.concat([exp_loc, pd.DataFrame([loc])], ignore_index=True)
                    exp_shape = pd.concat([exp_shape, pd.DataFrame([alpha])], ignore_index=True)
                    exp_scale = pd.concat([exp_scale, pd.DataFrame([beta])], ignore_index=True)
                    neg_bio_r = pd.concat([neg_bio_r, pd.DataFrame([r])], ignore_index=True)
                    neg_bio_p = pd.concat([neg_bio_p, pd.DataFrame([p])], ignore_index=True)


                    ks_t_D = pd.concat([ks_t_D, pd.DataFrame([D])], ignore_index=True)
                    ks_t_pval = pd.concat([ks_t_pval, pd.DataFrame([kspval])], ignore_index=True)
                    t_t_pval = pd.concat([t_t_pval, pd.DataFrame([pval / 2])], ignore_index=True)
                    # add timeslot
                    time_slot = pd.concat([time_slot, pd.DataFrame([var])], ignore_index=True)
                ...
                t_t_pval = pd.concat([t_t_pval, pd.DataFrame([pval / 2])], ignore_index=True)
                # add timeslot
                time_slot = pd.concat([time_slot, pd.DataFrame([f2])], ignore_index=True)
    ...
    def neg_bio_reg_fit_model(self,data,x):
        ...
        for f2 in np.unique(data_gamma['arrivalslot']):
            ...
            if self.combined_slots and f2 == self.combine[0]:
                for var in self.combine:
                    nb_mu = pd.concat([nb_mu, pd.DataFrame([mean])], ignore_index=True)
                    nb_p = pd.concat([nb_p, pd.DataFrame([p])], ignore_index=True)
                    nb_n = pd.concat([nb_n, pd.DataFrame([n])], ignore_index=True)
                    nb_alpha = pd.concat([nb_alpha, pd.DataFrame([alpha])], ignore_index=True)
                    time_slot = pd.concat([time_slot, pd.DataFrame([var])], ignore_index=True)
    ...
    def predict_poission_fit_mod(self,f1,f2,val='Poisson_lam',return_raw = False):
        ...
        if self._variablity_lambda and val =='Poisson_lam':
            ...
            model = self.pop_model(f1=f1)
            try:
                index = np.where(np.atleast_1d(np.asarray(model[2])) == f2)
