...
# Everything else (N_MIN=30, G_KMAX=6, L2_MC=2000, Random.seed!(@isdefined(SEED_RUN) ? SEED_RUN : 24), all logic)
...
Random.seed!(@isdefined(SEED_RUN) ? SEED_RUN : 24)
...
# ----- Data-driven gate for per-bin ENERGY fitting ---------------------------
# Energy is fitted per arrival-hour bin only if it actually depends on the bin.
# η² = fraction of E variance explained by the arrival-hour state. When η² is
# low (energy ⊥ arrival time, e.g. Georgia/Norway ≈ 0.04), per-bin energy fits
# just overfit noise on sparse states and hurt the per-user energy marginal
# (L1-E); we then fit E at the user/day-type level instead. Datasets where
# energy is genuinely arrival-dependent (GIST/Kim ≈ 0.17) keep per-bin fitting.
function eta_squared(vals::AbstractVector{<:Real}, groups)
    grand = mean(vals); sstot = sum((vals .- grand).^2)
    sstot == 0 && return 0.0
    ssb = 0.0
    for g in unique(groups)
        v = vals[groups .== g]
        ssb += length(v) * (mean(v) - grand)^2
    end
    return ssb / sstot
end
const ETA2_MIN_PERBIN = @isdefined(ETA2_MIN_PERBIN_RUN) ? ETA2_MIN_PERBIN_RUN : 0.10
eta2_E   = eta_squared(Float64.(pairs.E), pairs.bin_i)
E_PER_BIN = eta2_E >= ETA2_MIN_PERBIN
@printf("  η²(E|bin) = %.3f  → per-bin energy fitting: %s (threshold %.2f)\n",
        eta2_E, E_PER_BIN ? "ON" :
        "OFF — energy arrival-independent, using user-level fit", ETA2_MIN_PERBIN)
...
for (uid, dt) in unique(zip(pairs.user_id, pairs.day_type))
    ...
    for b in 1:NUM_BINS
        ...
        e1 = (E_PER_BIN && nrow(bin_rows) >= N_BIN_NOMINAL) ? fit_best(bin_rows.E) : nothing
        ...
        # When per-bin energy is gated off (E arrival-independent), E is pinned
        # to L2 by design, so it is excluded from the tally — otherwise it would
        # mask genuine per-bin D/G/A fits and report L1=0% spuriously.
        fallback[b] = E_PER_BIN ? max(lD, lG, lE, lA) : max(lD, lG, lA)
