...
class mixture_models:
    ...
    def fit_mm_EM(self,data,log=False,verbose=0):
        ...
        X_fit = X_fit[~np.isnan(X_fit)].reshape(-1, 1)
        means[~np.isfinite(means)] = 0
        precisions[~np.isfinite(precisions)] = 0.001
    ...
    def pop_model(self,f1,f2):
        ...
        factors = np.array(self._best_fit[self.x_names[0]])
        slots = np.array(self._best_fit[self.x_names[1]])
        Model = self._best_fit['Model']
        non_none = np.array([m is not None for m in Model])
        # exact match on (factor, slot) with fitted model
        selector_boolan = (factors == f1) & (slots == f2) & non_none
        selector = np.where(selector_boolan.flatten())[0]
        if len(selector) > 0:
            return Model[int(selector[0])]
        # fallback: same factor, nearest available slot with fitted model
        factor_selector = np.where(((factors == f1) & non_none).flatten())[0]
        if len(factor_selector) > 0:
            available_slots = slots.flatten()[factor_selector]
            nearest_idx = factor_selector[int(np.argmin(np.abs(available_slots - f2)))]
            return Model[nearest_idx]
        # last-resort fallback: any fitted model
        any_fit = np.where(non_none.flatten())[0]
        if len(any_fit) > 0:
            return Model[int(any_fit[0])]
        return None
    ...
    def predict_day(self, X_test, arrivals, slot,verbose=0):
        ...
        for arr in arrivals:
            ...
            arr_scalar = float(np.asarray(arr).flatten()[0])
            t_now_slot = int(np.asarray(get_slotted_data(data=arr_scalar, slot_secs=slot)).flatten()[0])

            dep = self.predict_mix(f1=np.array(X_test),f2=t_now_slot)

            deps.append(float(np.asarray(dep).flatten()[0]))
